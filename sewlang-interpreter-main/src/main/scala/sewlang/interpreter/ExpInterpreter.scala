...
object  ExpInterpreter {
  ...
  def eval(exp: Exp)(env: Environment): Value = exp match {
    ...
    case SubExp(exp1, exp2)            => evalArithExp(_ - _, eval(exp1)(env), eval(exp2)(env)) // (- exp1 exp2)
    case DivExp(exp1, exp2)            => evalArithExp(_ / _, eval(exp1)(env), eval(exp2)(env)) // (/ exp1 exp2)
    case NegExp(exp)                   => evalArithExp(_ - _, eval(exp)(env), NilV)             //  (- exp)
    ...
    case LessEqualExp(exp1, exp2)      => evalRelationalExp(_ <= _, eval(exp1)(env), eval(exp2)(env))  // (<= exp1 exp2)
    case BiggerThanExp(exp1, exp2)     => evalRelationalExp(_ > _, eval(exp1)(env), eval(exp2)(env))   // (> exp1 exp2)
    case BiggerEqualExp(exp1, exp2)    => evalRelationalExp(_ >= _, eval(exp1)(env), eval(exp2)(env))  // (>= exp1 exp2)
    ...
    case ReadBoolExp                   => BoolV(readBool())  //(read-bool)
    case ReadStrExp                    => StringV(readStr()) //(read-str)
  }
  ...
  private[interpreter] var readBool: () => Boolean = io.StdIn.readBoolean  //(read-bool)
  private[interpreter] var readStr: () => String = io.StdIn.readLine       //(read-str)
