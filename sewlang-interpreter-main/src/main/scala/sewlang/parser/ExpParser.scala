...
object ExpParser {
  ...
  def parse(sexp: SExpr): Exp = sexp match {
    ...
    case SList(List(SSym("-"),exp1)) => NegExp(parse(exp1))                     // (- exp)
    ...
    case SList(List(SSym("<="), exp1, exp2)) => LessEqualExp(parse(exp1), parse(exp2))  // (<= exp1 exp2)
    case SList(List(SSym(">"), exp1, exp2)) => BiggerEqualExp(parse(exp1), parse(exp2)) // (> exp1 exp2)
    case SList(List(SSym(">"), exp1, exp2)) => BiggerThanExp(parse(exp1), parse(exp2))  // (>= exp1 exp2)
    ...
    case SList(List(SSym("--"), id)) => parse(ExpDesugar.desugar(sexp)) //(-- id)
    ...
    case SList(List(SSym("-="), id, exp)) => parse(ExpDesugar.desugar(sexp)) //(-= id exp)
    case SList(List(SSym("*="), id, exp)) => parse(ExpDesugar.desugar(sexp)) //(*= id exp)
    case SList(List(SSym("/="), id, exp)) => parse(ExpDesugar.desugar(sexp)) //(/= id exp)
    ...
    case SList(List(SSym("while"), body, until-cond)) => parse(ExpDesugar.desugar(sexp))//repeat body until-cond
 ...
 */
  private def isIdentifier(str: String) = { //implementação do 9
    val regex = """^[a-zA-Z][a-zA-Z0-9_]*$""".r
    str match {
      case "while" => throw ExpParserException(s"[Erro]: invalid data")
